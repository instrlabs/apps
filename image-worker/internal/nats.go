...
func (n *NatsService) PublishJobNotification(jobID string, status JobStatus, userID string) error {
	msg := &JobNotificationMessage{ID: jobID, Status: status, UserID: userID}
	b, err := json.Marshal(msg)
	if err != nil {
		return err
	}
	return n.conn.Publish(n.cfg.NatsSubjectNotifications, b)
}

type ImageJobHandler func(ctx context.Context, job *JobMessage) error

func (n *NatsService) Subscribe(handler ImageJobHandler) error {
	_, err := n.conn.Subscribe(n.cfg.NatsSubjectRequests, func(msg *nats.Msg) {
		log.Printf("Received image job on subject: %s", n.cfg.NatsSubjectRequests)
		var job JobMessage
