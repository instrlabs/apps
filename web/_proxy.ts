...
export async function _proxy(req: NextRequest) {
  ...
  info(
    `Auth state - Access token: ${accessToken ? "present" : "missing"}, Refresh token: ${refreshToken ? "present" : "missing"}`,
    req,
  );
...
export const config = {
  matcher: ["/((?!api|debug|_next/static|_next/image|favicon.ico|\\.well-known|.*\\.png$).*)"],
};
